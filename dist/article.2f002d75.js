...
//    console.log(JSON.parse(localStorage.getItem("articleData")).time);
};
const popUp = (img_url, time, title, des, name, content)=>{
    document.querySelector("#pop__img").src = img_url;
    document.querySelector("#pop__time").innerHTML = time;
    ...
    document.querySelector("#pop__read-more").addEventListener("click", ()=>{
        renderArticle(time, title, name, img_url, content);
    });
            ...
            <div class="section col-12 feed--main-card-cont">
                 <div class="feed--main-card active">
                    ...
                    <div class="bottom-cont">
                        ...
                        <a class="btn btn-primary ms-auto btn-read__more" href="article.html" target="_blank">Read more</a>
                    </div>
                </div>
            </div>
        `;
...
const renderTrends = (parentEl)=>{
    ...
    for(let i = feed.articles.length - 1; i > feed.articles.length - 5; i--){
        ...
        document.querySelector(".btn-read").addEventListener("click", ()=>{
            popUp(feed.articles[i].urlToImage, feed.articles[i].publishedAt, feed.articles[i].title, feed.articles[i].description, feed.articles[i].author, feed.articles[i].content);
        });
...
document.querySelector("#art__time").innerHTML = storedData.time;
document.querySelector("#art__title").innerHTML = storedData.title;
...
//# sourceMappingURL=article.2f002d75.js.map
